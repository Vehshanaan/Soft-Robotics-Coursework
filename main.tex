...
\begin{figure}
    ...
    \caption{A fully stretched gripper with 90° tilted strain limited layer. The effect is trivial.}
    \label{fig:Trivial}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Level of effect}: Although the effect of strain limited layer is clear in the Fig. \ref{fig:OverallResult}, its rather trivial when observed by eyes (as shown in Fig. \ref{fig:Trivial}), which means the potential of physically programming with this technique is limited.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Other factors that effects the strain characteristic}: The manufacturing of the gripper could be divided into two stages: casting (casting the elastomer) and bonding (attaching the cloth). Mistakes happened in each of these stages could have significant influence on the strain characteristic.
        \begin{itemize}
            \item Casting stage: If some chambers have thinner walls than the others, they would be much more easier to expand and will significantly effect the strain characteristic.
            \item Bonding stage: If some of the chamber walls are not attached to the strain limited layer, causing two chambers partially merged into one, the strain characteristic would be significantly effected.
        \end{itemize}
        Though these two factors have great impact on the strain characteristic, they are not controllable with the toolbox given.
\end{itemize}
